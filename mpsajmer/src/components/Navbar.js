...
const Navbar = (props) => {
  ...
  const { fetchUser, authenticated } = context;
  const { setMode } = props;
  // const testUserApi = async () => {
  //   const response = await fetch(`${vars.host}/api/user-data/`, {
  //     method: "GET",
  //     headers: {
  //       "Authorization": `Token ${localStorage.getItem("MPSUser")}`,
  //     },
  //   });
  ...
  return (
    <>
      <nav
        id="navbar"
        className="fixed top-0 left-0 right-0 bg-white dark:bg-gray-800 dark:border-b-white shadow-xl z-10 text-black max-h-[4.7rem]"
      >
        <div className="flex items-center h-fit">
          <div
            className={`left block top-auto left-4 absolute ${
          ...
          >
            ...
            >
              <div
                className="pt-[4px] my-1 w-8 dark:bg-purple-600 bg-purple-500 transition-all duration-700 rounded-full"
                id="sideHamDiv1"
              />
              <div
                className="pt-[4px] my-1 w-8 dark:bg-purple-600 bg-purple-500 transition-all duration-700 rounded-full"
              ...
              />
              <div
                className="pt-[4px] my-1 w-8 dark:bg-purple-600 bg-purple-500 transition-all duration-200 rounded-full"
                id="sideHamDiv3"
              />
            </button>
          </div>
            ...
            <Link to="/">
              <div className="siteTitleImg flex items-center justify-center">
                ...
                <div className="logoTitle cursor-pointer">
                  <p className="dark:text-white text-purple-600 font-bold font-Kalnia lg:text-2xl md:text-lg">
                    {props.title}
                  </p>
                </div>
              </div>
          ...
          <div className="nav-right flex xl:space-x-2 md:space-x-1 items-center justify-center absolute right-0 md:mx-8 mx-2">
            ...
            <form
              role="search"
              onSubmit={searchFunc}
              className="search border-purple-600 md:mx-1 mr-1 hidden lg:block"
            >
              <input
                type="text"
                className="dark:bg-gray-700 dark:border-0 dark:focus:border-0 dark:text-white text-black px-3 lg:w-28 xl:w-44 w-32 py-1 rounded-lg inline border-2 transition-all duration-100 focus:border-4 lg:text-sm focus:border-purple-700 xl:text-lg lg:py-1 lg:px-2 xl:py-2 xl:px-4 border-purple-600"
                placeholder="Search..."
                name="query"
                id="querys"
                value={query}
            ...
            <div className="search md:mx-2 lg:hidden space-x-1 flex mr-1">
              <button onClick={toggleDarkMode} className="">
                <svg
                  ...
                  stroke-width="0"
                  viewBox="0 0 16 16"
                  className="md:hidden w-4 text-purple-700 mt-1 ml-2 cursor-pointer dark:text-white transition-all duration-300"
                  height="28"
                  width="28"
                  xmlns="http://www.w3.org/2000/svg"
                >
            ...
            {!authenticated ? (
              <div>
                <Link to="/login">
                  <button className="text-center rounded-xl hover:bg-purple-800 text-white bg-purple-600 lg:px-6 md:px-4 px-4 text-xl ml-2 md:text-xl pt-1 pb-1 login">
                    Login
                  </button>
                </Link>
              </div>
              ...
              //       alt=""
              //       className="md:h-12 h-8 w-8 md:w-12 rounded-full border-2 dark:border-white border-purple-600 mr-1 transition-all duration-500"
              //     />
              //     <img
              //       src={DropDownImg}
              //       alt=""
              ...
              <div>
                <button
                  onClick={logout}
                  className="text-center rounded-xl hover:bg-purple-800 text-white bg-purple-600 lg:px-6 md:px-4 px-4 text-xl ml-2 md:text-xl pt-1 pb-1 login"
                >
                  Logout
                </button>
              </div>
        ...
        >
          <form onSubmit={searchFunc}>
            <input
              type="text"
              className="w-full px-4 py-2 rounded-md border-4 border-purple-600 text-xl text-purple-700"
              placeholder="Search"
              name="query"
              id="queryMobile"
              value={query}
